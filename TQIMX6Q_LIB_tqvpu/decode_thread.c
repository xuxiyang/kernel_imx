...
void *decStreamThread(void)
{
   struct vpu_dec_handle *thandle = &handle;

   int ret,err;
   //thandle=(struct vpu_dec_handle*)malloc(sizeof(struct vpu_dec_handle));
...
main(int argc, char *argv[])
{
	...
	info_msg("VPU test program built on %s %s\n", __DATE__, __TIME__);
        ...
        if (check_params(&input_arg[0].cmd, input_arg[0].mode) == 0) {
           ...
           pthread_create(&decThread, NULL, decStreamThread, NULL);
