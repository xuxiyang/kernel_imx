...
//extern int 	/* write n bytes to a file descriptor */
//fwriten(int fd, void *vptr, size_t n);

        /* write n bytes to a file descriptor */
//int __weak fwriten(int fd, void *vptr, size_t n)
//{
//        int nleft;
//        int nwrite;
//        char  *ptr;
...
#else
        return decoder_get_1frame_data_inputCallBack(fd, buf, n);
#endif
}
...
vpu_write(struct cmd_line *cmd, char *buf, int n)
{
	...
	if (cmd->dst_scheme == PATH_NET) {
        ...
        return encoder_deal_with_1frame_data_outputCallBack(fd, buf, n);
}
...
#endif
        } else if(cmd->src_scheme == PATH_BUFFER){
            info_msg("The input file is not specified and enters the frame mode!\n");
...
check_params(struct cmd_line *cmd, int op)
{
	...
	if (cmd->port == 0) {
        ...
        if (cmd->src_scheme != PATH_FILE && cmd->src_scheme != PATH_BUFFER && op == DECODE) {
		cmd->src_scheme = PATH_NET;
	}

        if (cmd->src_scheme == PATH_FILE && cmd->src_scheme != PATH_BUFFER && op == ENCODE) {
