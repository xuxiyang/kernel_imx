...
#include "../include/vpu_jpegtable.h"
...
struct enc_argst{
    ...
    int src_scheme;
    int count;
    struct timeval tenc_begin;
    struct timeval tenc_end;
    ...
    float tenc_time;
    float total_time;
    PhysicalAddress phy_bsbuf_start;
    u32 virt_bsbuf_start;
    u32 virt_bsbuf_end;
    int is_waited_int;
};
struct enc_argst enc_st;
static struct cmd_line *cmdl = NULL;
static vpu_mem_desc mem_desc = {0};
static vpu_mem_desc scratch_mem_desc = {0};
static struct encode *enc = NULL;

static struct vpu_enc_handle encHandle;
typedef enum vpu_enc_beacon{
    ENC_DEFAULT = 0,
    ENC_INIT,
    ENC_LOOP,
    ENC_COMPLETE,
...
static vpu_enc_beacon_en enc_beacon = ENC_INIT;
...
read_from_file(struct encode *enc)
{
    ...
    if (img_size == pfb->desc.size) {
        ret = encoder_get_1frame_data_inputCallBack(src_fd, (void *)y_addr, img_size);
    } else {
        ret = encoder_get_1frame_data_inputCallBack(src_fd, (void *)y_addr, y_size);
        if (chromaInterleave == 0) {
            ret = encoder_get_1frame_data_inputCallBack(src_fd, (void *)u_addr, c_size);
            ret = encoder_get_1frame_data_inputCallBack(src_fd, (void *)v_addr, c_size);
        } else {
            ret = encoder_get_1frame_data_inputCallBack(src_fd, (void *)u_addr, c_size * 2);
        }
    }
    return ret;
}
...
void enc_argst_init(struct enc_argst *argst)
{
    struct enc_argst iargst={
        .handle = enc->handle,
        .enc_param = {0},
        .encop = {0},
        .outinfo = {0},
        .ret = 0,
        .src_fbid = enc->src_fbid,
                .img_size = 0,
                .frame_id = 0,
        .fb = enc->fb,
        .v4l2_buf={0},
        .src_scheme = enc->cmdl->src_scheme,
        .count = enc->cmdl->count,
        .tenc_begin={0},
                .tenc_end={0},
                .total_start={0},
                .total_end={0},
        ...
        .tenc_time = 0,
                .total_time=0,
        .phy_bsbuf_start = enc->phy_bsbuf_addr,
        .virt_bsbuf_start = enc->virt_bsbuf_addr,
        .virt_bsbuf_end = enc->virt_bsbuf_addr + STREAM_BUF_SIZE,
        .is_waited_int = 0,
    };
    memcpy((struct dec_argst *)argst,(struct dec_argst *)&iargst,sizeof(iargst));
}
static int encoder_process(void)
{
    while(1){
        if (enc_st.src_scheme == PATH_V4L2) {
                enc_st.ret = v4l_get_capture_data(&enc_st.v4l2_buf);
                if (enc_st.ret < 0) {
                        return -1;//goto err2;;
                }

                enc_st.fb[enc_st.src_fbid].myIndex = enc->src_fbid + enc_st.v4l2_buf.index;
                enc_st.fb[enc_st.src_fbid].bufY = cap_buffers[enc_st.v4l2_buf.index].offset;
                enc_st.fb[enc_st.src_fbid].bufCb = enc_st.fb[enc_st.src_fbid].bufY + enc_st.img_size;
                if ((enc->cmdl->format == STD_MJPG) &&
                    (enc->mjpg_fmt == MODE422 || enc->mjpg_fmt == MODE224))
                        enc_st.fb[enc_st.src_fbid].bufCr = enc_st.fb[enc_st.src_fbid].bufCb + (enc_st.img_size >> 1);
                else
                        enc_st.fb[enc_st.src_fbid].bufCr = enc_st.fb[enc_st.src_fbid].bufCb + (enc_st.img_size >> 2);
                enc_st.fb[enc_st.src_fbid].strideY = enc->src_picwidth;
                enc_st.fb[enc_st.src_fbid].strideC = enc->src_picwidth / 2;
        } else {
                enc_st.ret = read_from_file(enc);
                if (enc_st.ret <= 0)
                        break;
        }
        ...
        if (cpu_is_mx6x() && (enc->cmdl->format == STD_MJPG)) {
                enc_st.ret = encoder_fill_headers(enc);
                if (enc_st.ret) {
        ...
        gettimeofday(&enc_st.tenc_begin, NULL);
        enc_st.ret = vpu_EncStartOneFrame(enc_st.handle, &enc_st.enc_param);
        if (enc_st.ret != RETCODE_SUCCESS) {
                err_msg("vpu_EncStartOneFrame failed Err code:%d\n",
                                                                enc_st.ret);
                return -1;//goto err2;;
        }

        enc_st.is_waited_int = 0;
        enc_st.loop_id = 0;
        while (vpu_IsBusy()) {
                if (enc->ringBufferEnable == 1) {
                        if (cpu_is_mx6x() && enc->cmdl->format == STD_MJPG)
                                enc_st.ret = enc_readbs_ring_buffer(enc_st.handle, enc->cmdl,
                                                enc_st.virt_bsbuf_start, enc_st.virt_bsbuf_end,
                                                enc_st.phy_bsbuf_start, 0);
                        else
                                enc_st.ret = enc_readbs_ring_buffer(enc_st.handle, enc->cmdl,
                                                enc_st.virt_bsbuf_start, enc_st.virt_bsbuf_end,
                                                enc_st.phy_bsbuf_start, STREAM_READ_SIZE);
                        if (enc_st.ret < 0) {
                                return -1;//goto err2;;
                        }
                }
                if (enc_st.loop_id == 20) {
                        enc_st.ret = vpu_SWReset(enc_st.handle, 0);
                        return -1;
                }
                if (vpu_WaitForInt(200) == 0)
                        enc_st.is_waited_int = 1;
                enc_st.loop_id ++;
        }

        if (!enc_st.is_waited_int)
                vpu_WaitForInt(200);

        gettimeofday(&enc_st.tenc_end, NULL);
        enc_st.sec = enc_st.tenc_end.tv_sec - enc_st.tenc_begin.tv_sec;
        enc_st.usec = enc_st.tenc_end.tv_usec - enc_st.tenc_begin.tv_usec;

        if (enc_st.usec < 0) {
                enc_st.sec--;
                enc_st.usec = enc_st.usec + 1000000;
        }

        enc_st.tenc_time += (enc_st.sec * 1000000) + enc_st.usec;

        enc_st.ret = vpu_EncGetOutputInfo(enc_st.handle, &enc_st.outinfo);

        usleep(0);

        dprintf(3, "enc_st.frame_id %d\n", (int)enc_st.frame_id);
        ...
        if (enc_st.outinfo.skipEncoded)
                info_msg("Skip encoding one Frame!\n");

        if (enc_st.outinfo.mbInfo.enable && enc_st.outinfo.mbInfo.size && enc_st.outinfo.mbInfo.addr) {
                SaveEncMbInfo(enc_st.outinfo.mbInfo.addr, enc_st.outinfo.mbInfo.size,
                                 enc_st.encop.picWidth/16, enc_st.frame_id);
        }

        if (enc_st.outinfo.mvInfo.enable && enc_st.outinfo.mvInfo.size && enc_st.outinfo.mvInfo.addr) {
                SaveEncMvInfo(enc_st.outinfo.mvInfo.addr, enc_st.outinfo.mvInfo.size,
                                 enc_st.encop.picWidth/16, enc_st.frame_id);
        }

        if (enc_st.outinfo.sliceInfo.enable && enc_st.outinfo.sliceInfo.size &&
            enc_st.outinfo.sliceInfo.addr) {
                SaveEncSliceInfo(enc_st.outinfo.sliceInfo.addr,
                                     enc_st.outinfo.sliceInfo.size, enc_st.frame_id);
        }

        if (enc_st.src_scheme == PATH_V4L2) {
                v4l_put_capture_data(&enc_st.v4l2_buf);
        }
        ...
        if (enc->ringBufferEnable == 0) {
                enc_st.ret = enc_readbs_reset_buffer(enc, enc_st.outinfo.bitstreamBuffer, enc_st.outinfo.bitstreamSize);
                if (enc_st.ret < 0) {
                        err_msg("writing bitstream buffer failed\n");
                        return -1;//goto err2;;
                }
        } else
                enc_readbs_ring_buffer(enc_st.handle, enc->cmdl, enc_st.virt_bsbuf_start,
                                        enc_st.virt_bsbuf_end, enc_st.phy_bsbuf_start, 0);

        enc_st.frame_id++;
        if ((enc_st.count != 0) && (enc_st.frame_id >= enc_st.count))
                break;
    }
}
int
encoder_start(void)
{
    ...
    case ENC_INIT:
        ...
        enc_st.enc_param.sourceFrame = &enc->fb[enc_st.src_fbid];
        enc_st.enc_param.quantParam = 23;
        enc_st.enc_param.forceIPicture = 0;
        enc_st.enc_param.skipPicture = 0;
        enc_st.enc_param.enableAutoSkip = 1;

        enc_st.enc_param.encLeftOffset = 0;
        enc_st.enc_param.encTopOffset = 0;
        if ((enc_st.enc_param.encLeftOffset + enc->enc_picwidth) > enc->src_picwidth) {
            err_msg("Configure is failure for width and left offset\n");
            return -1;
        }
        if ((enc_st.enc_param.encTopOffset + enc->enc_picheight) > enc->src_picheight) {
        ...
        if (enc->mbInfo.enable) {
            enc_st.ret = vpu_EncGiveCommand(enc_st.handle, ENC_SET_REPORT_MBINFO, &enc->mbInfo);
            if (enc_st.ret != RETCODE_SUCCESS) {
                err_msg("Failed to set MbInfo report, enc_st.ret %d\n", enc_st.ret);
                return -1;
            }
        }
        if (enc->mvInfo.enable) {
            enc_st.ret = vpu_EncGiveCommand(enc_st.handle, ENC_SET_REPORT_MVINFO, &enc->mvInfo);
            if (enc_st.ret != RETCODE_SUCCESS) {
                err_msg("Failed to set MvInfo report, enc_st.ret %d\n", enc_st.ret);
                return -1;
            }
        }
        if (enc->sliceInfo.enable) {
            enc_st.ret = vpu_EncGiveCommand(enc_st.handle, ENC_SET_REPORT_SLICEINFO, &enc->sliceInfo);
            if (enc_st.ret != RETCODE_SUCCESS) {
                err_msg("Failed to set slice info report, enc_st.ret %d\n", enc_st.ret);
                return -1;
            }
        }
        if (enc_st.src_scheme == PATH_V4L2) {
            enc_st.ret = v4l_start_capturing();
            if (enc_st.ret < 0) {
                return -1;
            }
            enc_st.img_size = enc->src_picwidth * enc->src_picheight;
        } else {
            enc_st.img_size = enc->src_picwidth * enc->src_picheight * 3 / 2;
            if (enc->cmdl->format == STD_MJPG) {
                if (enc->mjpg_fmt == MODE422 || enc->mjpg_fmt == MODE224)
                    enc_st.img_size = enc->src_picwidth * enc->src_picheight * 2;
                else if (enc->mjpg_fmt == MODE400)
                    enc_st.img_size = enc->src_picwidth * enc->src_picheight;
            }
        }

        gettimeofday(&enc_st.total_start, NULL);
        enc_beacon = ENC_INIT;
    case ENC_LOOP:
        /* The main encoding loop */
        if(encoder_process()<0)
            goto err2;
        enc_beacon = ENC_COMPLETE;
    case ENC_COMPLETE:
        gettimeofday(&enc_st.total_end, NULL);
        enc_st.sec = enc_st.total_end.tv_sec - enc_st.total_start.tv_sec;
        enc_st.usec = enc_st.total_end.tv_usec - enc_st.total_start.tv_usec;
        if (enc_st.usec < 0) {
            enc_st.sec--;
            enc_st.usec = enc_st.usec + 1000000;
        }
        enc_st.total_time = (enc_st.sec * 1000000) + enc_st.usec;

        info_msg("Finished encoding: %d frames\n", enc_st.frame_id);
        info_msg("enc fps = %.2f\n", (enc_st.frame_id / (enc_st.tenc_time / 1000000)));
        info_msg("total fps= %.2f \n",(enc_st.frame_id / (enc_st.total_time / 1000000)));

        enc_beacon = ENC_CLOSE;
...
int
encoder_init(struct vpu_enc_handle *handle)
{
    int ret;
    cmdl = handle->cmdl;
    enc = handle->enc;
    mem_desc = handle->mem_desc;
    scratch_mem_desc=handle->scratch_mem_desc;
